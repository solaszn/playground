...
const Features = () => {
  ...
  return (
    <TabGroup selectedIndex={activeTab} onChange={handleTabChange} className="w-full mt-12 px-6">
      <TabList className="flex gap-4 px-2 py-1.5 rounded-full bg-gray-100 w-fit mx-auto">
        {tabs.map((tab: any) => (
      ...
      <TabPanels className="w-full mt-10 ">
        <div className="aspect-video bg-gray-100 w-10/12 mx-auto rounded-2xl overflow-clip relative">
          <div className="relative zu0vm9576q w-full h-full flex items-end z-20 p-6 lg:p-10">
            <div className="flex flex-col gap-2 w-full sm:w-9/12 lg:w-1/2">
              <div className="text-2xl font-semibold text-white">{tabs[activeTab].label}</div>
              <div className="text-gray-200">{tabs[activeTab].message}</div>
            </div>
          </div>
          <AnimatePresence initial={false} custom={getDirection()}>
            {/* Previous Image (Fading Out) */}
            <motion.img
              key={tabs[lastTab].label}
